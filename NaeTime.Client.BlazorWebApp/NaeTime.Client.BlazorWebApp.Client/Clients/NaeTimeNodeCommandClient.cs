...
public class NaeTimeNodeCommandClient : INaeTimeNodeCommandHandler
{
    ...
    {
        var url = $"/node/configure-serial-esp32?id={id}&name={Uri.EscapeDataString(name)}&port={Uri.EscapeDataString(port)}&lanes={lanes}";
        return PostNoContentAsync(url);
    }

    public Task ReconfigureSerialNode(Guid id, string port)
    {
        var url = $"/node/reconfigure-serial?id={id}&port={Uri.EscapeDataString(port)}";
        return PostNoContentAsync(url);
    }

    public Task RenameDevice(Guid id, string name)
    {
        var url = $"/node/rename?id={id}&name={Uri.EscapeDataString(name)}";
        return PostNoContentAsync(url);
    }

    public Task RequestEnableLane(Guid id, byte lane)
    {
        var url = $"/node/request-enable-lane?id={id}&lane={lane}";
        return PostNoContentAsync(url);
    }

    public Task RequestDisableLane(Guid id, byte lane)
    {
        var url = $"/node/request-disable-lane?id={id}&lane={lane}";
        return PostNoContentAsync(url);
    }

    public Task ConfirmLaneEnabled(Guid id, byte lane)
    {
        var url = $"/node/confirm-enabled?id={id}&lane={lane}";
        return PostNoContentAsync(url);
    }

    public Task ConfirmLaneDisabled(Guid id, byte lane)
    {
        var url = $"/node/confirm-disabled?id={id}&lane={lane}";
        return PostNoContentAsync(url);
    }
    ...
    public Task RequestLaneFrequency(Guid id, byte lane, byte? bandId, int frequencyInMHz)
    {
        ...
        sb.Append($"/node/request-frequency?id={id}&lane={lane}&frequencyInMHz={frequencyInMHz}");
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.Append($"/node/confirm-frequency?id={id}&lane={lane}&frequencyInMHz={frequencyInMHz}");
        if (bandId.HasValue) sb.Append($"&bandId={bandId.Value}");
        return PostNoContentAsync(sb.ToString());
    }
    ...
    {
        var url = $"/node/request-entry-threshold?id={id}&lane={lane}&threshold={threshold}";
        return PostNoContentAsync(url);
    }

    public Task ConfirmLaneEntryThresholdConfigured(Guid id, byte lane, ushort threshold)
    {
        var url = $"/node/confirm-entry-threshold?id={id}&lane={lane}&threshold={threshold}";
        return PostNoContentAsync(url);
    }

    public Task RequestLaneExitThreshold(Guid id, byte lane, ushort threshold)
    {
        var url = $"/node/request-exit-threshold?id={id}&lane={lane}&threshold={threshold}";
        return PostNoContentAsync(url);
    }

    public Task ConfirmLaneExitThresholdConfigured(Guid id, byte lane, ushort threshold)
    {
        var url = $"/node/confirm-exit-threshold?id={id}&lane={lane}&threshold={threshold}";
        return PostNoContentAsync(url);
    }
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.Append($"/node/mark-lane-rf-setup-read?id={id}&lane={lane}&isEnabled={isEnabled.ToString().ToLowerInvariant()}&frequencyInMHz={frequencyInMHz}&entryThreshold={entryThreshold}&exitThreshold={exitThreshold}");
        if (bandId.HasValue) sb.Append($"&bandId={bandId.Value}");
        return PostNoContentAsync(sb.ToString());
    }
    ...
    {
        var url = $"/node/request-lane-rf-setup-confirmation?id={id}&lane={lane}";
        return PostNoContentAsync(url);
    }

    public Task MarkLaneRFSetupConfirmed(Guid id, byte lane)
    {
        var url = $"/node/mark-lane-rf-setup-confirmed?id={id}&lane={lane}";
        return PostNoContentAsync(url);
    }

    public Task MarkLaneRFSetupMismatch(Guid id, byte lane)
    {
        var url = $"/node/mark-lane-rf-setup-mismatch?id={id}&lane={lane}";
        return PostNoContentAsync(url);
    }

    public Task EnableLaneRFSetupSync(Guid id, byte lane)
    {
        var url = $"/node/enable-lane-rf-setup-sync?id={id}&lane={lane}";
        return PostNoContentAsync(url);
    }

    public Task DisableLaneRFSetupSync(Guid id, byte lane)
    {
        var url = $"/node/disable-lane-rf-setup-sync?id={id}&lane={lane}";
        return PostNoContentAsync(url);
    }

    public Task RequestTimerRFSetupConfirmation(Guid id)
    {
        var url = $"/node/request-timer-rf-setup-confirmation?id={id}";
        return PostNoContentAsync(url);
    }

    public Task MarkTimerRFSetupConfirmed(Guid id)
    {
        var url = $"/node/mark-timer-rf-setup-confirmed?id={id}";
        return PostNoContentAsync(url);
    }

    public Task MarkTimerRFSetupMismatch(Guid id)
    {
        var url = $"/node/mark-timer-rf-setup-mismatch?id={id}";
        return PostNoContentAsync(url);
    }

    public Task MarkAsConnected(Guid id)
    {
        var url = $"/node/mark-connected?id={id}";
        return PostNoContentAsync(url);
    }

    public Task MarkAsDisconnected(Guid id)
    {
        var url = $"/node/mark-disconnected?id={id}";
        return PostNoContentAsync(url);
    }
}
