...
public class OpenPracticeCommandClient : IOpenPracticeCommandHandler
{
    ...
    {
        var url = $"/openpractice/schedule?id={id}&trackId={trackId}&name={Uri.EscapeDataString(name)}";
        return PostNoContentAsync(url);
    }

    public Task CloneSession(Guid newId, Guid existingId, string newName)
    {
        var url = $"/openpractice/clone?newId={newId}&existingId={existingId}&newName={Uri.EscapeDataString(newName)}";
        return PostNoContentAsync(url);
    }

    public Task CloneSessionOnNewTrack(Guid newId, Guid existingId, string newName, Guid trackId)
    {
        var url = $"/openpractice/clone-newtrack?newId={newId}&existingId={existingId}&newName={Uri.EscapeDataString(newName)}&trackId={trackId}";
        return PostNoContentAsync(url);
    }

    public Task RenameSession(Guid sessionId, string name)
    {
        var url = $"/openpractice/rename?sessionId={sessionId}&name={Uri.EscapeDataString(name)}";
        return PostNoContentAsync(url);
    }

    public Task DisableLane(Guid sessionId, byte lane)
    {
        var url = $"/openpractice/disable-lane?sessionId={sessionId}&lane={lane}";
        return PostNoContentAsync(url);
    }

    public Task EnableLane(Guid sessionId, byte lane)
    {
        var url = $"/openpractice/enable-lane?sessionId={sessionId}&lane={lane}";
        return PostNoContentAsync(url);
    }
    ...
    public Task TuneLane(Guid sessionId, byte lane, byte? bandId, int frequencyInMhz)
    {
        ...
        sb.Append($"/openpractice/tune-lane?sessionId={sessionId}&lane={lane}&frequencyInMhz={frequencyInMhz}");
    ...
    {
        var url = $"/openpractice/set-lane-pilot?sessionId={sessionId}&lane={lane}&pilotId={pilotId}";
        return PostNoContentAsync(url);
    }

    public Task ResetLanePilot(Guid sessionId, byte lane)
    {
        var url = $"/openpractice/reset-lane-pilot?sessionId={sessionId}&lane={lane}";
        return PostNoContentAsync(url);
    }
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.Append($"/openpractice/assign-detection?detectionId={detectionId}&sessionId={sessionId}&timerId={timerId}&lane={lane}");
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.Append($"/openpractice/trigger-detection?detectionId={detectionId}&sessionId={sessionId}&lane={lane}&ordinalPosition={ordinalPosition}");
        if (hardwareTime.HasValue) sb.Append($"&hardwareTime={hardwareTime.Value}");
        sb.Append($"&softwareTime={softwareTime}&utcTime={Uri.EscapeDataString(utcTime.ToString("o"))}");
        return PostNoContentAsync(sb.ToString());
    }
    ...
    {
        var url = $"/openpractice/invalidate-detection?sessionId={sessionId}&detectionId={detectionId}";
        return PostNoContentAsync(url);
    }

    public Task InvalidateAllPilotDetections(Guid sessionId, Guid pilotId)
    {
        var url = $"/openpractice/invalidate-all-pilot-detections?sessionId={sessionId}&pilotId={pilotId}";
        return PostNoContentAsync(url);
    }

    public Task InvalidatePilotDetectionsBeforeDetection(Guid sessionId, Guid detectionId)
    {
        var url = $"/openpractice/invalidate-pilot-detections-before?sessionId={sessionId}&detectionId={detectionId}";
        return PostNoContentAsync(url);
    }

    public Task ValidateDetection(Guid detectionId, Guid sessionId)
    {
        var url = $"/openpractice/validate-detection?detectionId={detectionId}&sessionId={sessionId}";
        return PostNoContentAsync(url);
    }

    public Task InsertPilotPackEndBeforeDetection(Guid sessionId, Guid detectionId)
    {
        var url = $"/openpractice/insert-pack-end-before?sessionId={sessionId}&detectionId={detectionId}";
        return PostNoContentAsync(url);
    }

    public Task InsertPilotPackEndAfterDetection(Guid sessionId, Guid detectionId)
    {
        var url = $"/openpractice/insert-pack-end-after?sessionId={sessionId}&detectionId={detectionId}";
        return PostNoContentAsync(url);
    }

    public Task RemovePilotPackEnd(Guid sessionId, Guid packEndId)
    {
        var url = $"/openpractice/remove-pack-end?sessionId={sessionId}&packEndId={packEndId}";
        return PostNoContentAsync(url);
    }
}
