...
public class ImmersionRCCommandClient : IImmersionRCLapRFCommandHandler
{
    ...
    {
        var url = $"/api/immersionrc/register?id={id}&name={Uri.EscapeDataString(name)}&address={Uri.EscapeDataString(address.ToString())}&port={port}";
        return PostNoContentAsync(url);
    }

    public Task ReconfigureNetworkDevice(Guid id, IPAddress address, ushort port)
    {
        var url = $"/api/immersionrc/reconfigure?id={id}&address={Uri.EscapeDataString(address.ToString())}&port={port}";
        return PostNoContentAsync(url);
    }

    public Task RenameDevice(Guid id, string name)
    {
        var url = $"/api/immersionrc/rename?id={id}&name={Uri.EscapeDataString(name)}";
        return PostNoContentAsync(url);
    }
    ...
    public Task SetupLaneForSession(Guid id, byte lane, bool isEnabled, byte? bandId, int frequencyInMHz)
    {
        ...
        sb.Append($"/api/immersionrc/setup-lane?id={id}&lane={lane}&isEnabled={isEnabled.ToString().ToLowerInvariant()}&frequencyInMHz={frequencyInMHz}");
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.Append($"/api/immersionrc/confirm-lane?id={id}&lane={lane}&isEnabled={isEnabled.ToString().ToLowerInvariant()}&frequencyInMHz={frequencyInMHz}&threshold={threshold.ToString(CultureInfo.InvariantCulture)}&gain={gain}");
        if (bandId.HasValue) sb.Append($"&bandId={bandId.Value}");
        return PostNoContentAsync(sb.ToString());
    }
    ...
    {
        var url = $"/api/immersionrc/request-lane-status?id={id}&lane={lane}&isEnabled={isEnabled.ToString().ToLowerInvariant()}";
        return PostNoContentAsync(url);
    }

    public Task ConfirmLaneStatus(Guid id, byte lane, bool isEnabled)
    {
        var url = $"/api/immersionrc/confirm-lane-status?id={id}&lane={lane}&isEnabled={isEnabled.ToString().ToLowerInvariant()}";
        return PostNoContentAsync(url);
    }
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.Append($"/api/immersionrc/request-frequency?id={id}&lane={lane}&frequencyInMHz={frequencyInMHz}");
        if (bandId.HasValue) sb.Append($"&bandId={bandId.Value}");
        return PostNoContentAsync(sb.ToString());
    }
    ...
    {
        var sb = new System.Text.StringBuilder();
        sb.Append($"/api/immersionrc/confirm-frequency?id={id}&lane={lane}&frequencyInMHz={frequencyInMHz}");
        if (bandId.HasValue) sb.Append($"&bandId={bandId.Value}");
        return PostNoContentAsync(sb.ToString());
    }
    ...
    {
        var url = $"/api/immersionrc/request-threshold?id={id}&lane={lane}&threshold={threshold.ToString(CultureInfo.InvariantCulture)}";
        return PostNoContentAsync(url);
    }

    public Task ConfirmLaneThresholdConfigured(Guid id, byte lane, float threshold)
    {
        var url = $"/api/immersionrc/confirm-threshold?id={id}&lane={lane}&threshold={threshold.ToString(CultureInfo.InvariantCulture)}";
        return PostNoContentAsync(url);
    }

    public Task RequestLaneGain(Guid id, byte lane, ushort gain)
    {
        var url = $"/api/immersionrc/request-gain?id={id}&lane={lane}&gain={gain}";
        return PostNoContentAsync(url);
    }

    public Task ConfirmLaneGainConfigured(Guid id, byte lane, ushort gain)
    {
        var url = $"/api/immersionrc/confirm-gain?id={id}&lane={lane}&gain={gain}";
        return PostNoContentAsync(url);
    }

    public Task MarkAsConnected(Guid id)
    {
        var url = $"/api/immersionrc/mark-connected?id={id}";
        return PostNoContentAsync(url);
    }

    public Task MarkAsDisconnected(Guid id)
    {
        var url = $"/api/immersionrc/mark-disconnected?id={id}";
        return PostNoContentAsync(url);
    }
}
