...
public static class HardwareQueryEndpoints
{
    ...
    {
        // GET hardware/detector/{id}
        app.MapGet("/hardware/detector/{id:guid}", async (Guid id, IHardwareQueryHandler handler) =>
        {
        ...
        // GET hardware/detectors
        app.MapGet("/hardware/detectors", async (IHardwareQueryHandler handler) =>
        {
            var detectors = await handler.GetAllDetectors().ConfigureAwait(false);
            return Results.Ok(detectors);
        });

        // GET hardware/detectors/ids?ids=guid1,guid2,...
        app.MapGet("/hardware/detectors/ids", async (HttpRequest req, IHardwareQueryHandler handler) =>
        {
        ...
        // GET hardware/ethernetlaprf8channel/{id}
        app.MapGet("/hardware/ethernetlaprf8channel/{id:guid}", async (Guid id, IHardwareQueryHandler handler) =>
        {
        ...
        // GET hardware/immersionrclaprflane/{timerId}/{laneId}
        app.MapGet("/hardware/immersionrclaprflane/{timerId:guid}/{laneId:int}", async (Guid timerId, int laneId, IHardwareQueryHandler handler) =>
        {
        ...
        // GET hardware/immersionrclaprf/all
        app.MapGet("/hardware/immersionrclaprf/all", async (IHardwareQueryHandler handler) =>
        {
            var list = await handler.GetAllImmersionRCLapRFs().ConfigureAwait(false);
            return Results.Ok(list);
        });

        // GET hardware/immersionrclaprf/active/{timerId}
        app.MapGet("/hardware/immersionrclaprf/active/{timerId:guid}", async (Guid timerId, IHardwareQueryHandler handler) =>
        {
            var config = await handler.GetActiveImmersionRCLapRFLanesConfiguration(timerId).ConfigureAwait(false);
            return Results.Ok(config);
        });

        // GET hardware/timer/details/{timerId}
        app.MapGet("/hardware/timer/details/{timerId:guid}", async (Guid timerId, IHardwareQueryHandler handler) =>
        {
            var details = await handler.GetDetails(timerId).ConfigureAwait(false);
            return details is null ? Results.NotFound() : Results.Ok(details);
        });

        // GET hardware/timer/{timerId}/lane/{laneId}/details
        app.MapGet("/hardware/timer/{timerId:guid}/lane/{laneId:int}/details", async (Guid timerId, int laneId, IHardwareQueryHandler handler) =>
        {
            if (laneId < byte.MinValue || laneId > byte.MaxValue)
            {
                return Results.BadRequest($"laneId must be between {byte.MinValue} and {byte.MaxValue}.");
