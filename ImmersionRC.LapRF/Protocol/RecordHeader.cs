...
internal struct RecordHeader
{
    ...
    public static void SetRecordLength(byte[] buffer, ushort recordLength)
    {
        BitConverter.GetBytes(recordLength).CopyTo(buffer, LENGTH_START_INDEX);
    }

    public static void SetRecordCRC(byte[] buffer, ushort crc)
    {
        BitConverter.GetBytes(crc).CopyTo(buffer, CRC_START_INDEX);
    }
}
